...
const app = express();
const playerDB = new Database();
const topPlayersDB = new Database();
...
function UpdateUserData(steam64) {  // steam64 MUST be a string to avoid having to deal with large numbers in JS.
  fetch(proxy + "https://superiorservers.co/api/profile/" + steam64 + "/").then(data=> data.json()).then(async (playerInfo) => {
          ...
          const playerDBInfo = await playerDB.get(playerInfo.SteamID64);
          if(playerDBInfo == null) { // First Visit
                // Create an entirely new JSON profile
                playerDB.set(playerInfo.SteamID64, jsonUtil.createJSON(currentTimeStamp, playerInfo.DarkRP.Money, playerInfo.DarkRP.Karma, playerInfo.Badmin.Name))
          } else {
                // Add to prexisting JSON profile
                playerDB.set(playerInfo.SteamID64, jsonUtil.addEntry(playerDBInfo, currentTimeStamp, playerInfo.DarkRP.Money, playerInfo.DarkRP.Karma, playerInfo.Badmin.Name))
          }
    }).catch((err) => { 
...
async function UpdateAllUserData() {
  const data = await playerDB.getAll();
  const keyNames = Object.keys(data); // Keynames are steam64 ids.
  UpdateTopPlayersList();
  for (let i = 0; i < keyNames.length; i++) {
      if(top100SteamIDs.includes(keyNames[i])) {
        continue; // Deal with players in the top 100 after - purely for organisation of code
      }
    UpdateUserData(keyNames[i]);
  }
...
async function UpdateTopPlayersList() {
  ...
  const leaderBoardData = await fetch(proxy + "https://superiorservers.co/api/darkrp/leaderboard/money/").then(data=> data.json())
    for(var i = 0; i < leaderBoardData.Data.length; i ++) {
          ...
          top100SteamIDs.push(playerInfo.SteamID64);
          ...
          const currentTimeStamp = Math.round(Date.now()/1000); // UNIX seconds
          const playerDBInfo = await topPlayersDB.get(playerInfo.SteamID64);
          if(playerDBInfo == null) { // First Visit
                // Create an entirely new JSON profile
                topPlayersDB.set(playerInfo.SteamID64, jsonUtil.createJSON(currentTimeStamp, playerInfo.Money, playerInfo.Karma, playerInfo.Name))
          } else {
                // Add to prexisting JSON profile
                topPlayersDB.set(playerInfo.SteamID64, jsonUtil.addEntry(playerDBInfo, currentTimeStamp, playerInfo.Money, playerInfo.Karma, playerInfo.Name))
          }
    }
...
UpdateAllUserData();
setInterval(UpdateAllUserData, 1800000); // Every 30 mins


app.get('/api/player/:steamid', async (req, res) => {
    const playerData = await playerDB.get(req.params.steamid);
    if(playerData == null) {
...
app.get('/api/topplayers/', async (req, res) => {
    const topPlayersData = await topPlayersDB.getAll();
    if(topPlayersData == null) {
          res.send({Response: Responses.NoData});
    } else {
      res.send({Response: Responses.Success, Data: topPlayersData});
    }
});
app.get('/api/clear/:steamid', async (req, res) => {
  playerDB.delete(req.params.steamid)
